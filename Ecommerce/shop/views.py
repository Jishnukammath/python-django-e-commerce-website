...
def shop(request,c_slug=None):
    ...
    if c_slug!=None:
        ...
        prodt=products.objects.all().filter(category=c_page,avial=True)
    else:
         prodt=products.objects.all().filter(avial=True)
    ...
    return render(request,'index.html',{'pr':prodt,'categ':cat,'pg':pro})
